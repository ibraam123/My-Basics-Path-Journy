...
public:
	...
	{
		return  to_string(Date.GetDay()) + "/" + to_string(Date.GetMonth()) + "/" + to_string(Date._Year);
	}
	static string DateinHMS(clsDate Date){
		return to_string(Date.GetHour()) +  ":" + to_string(Date.GetMinute()) + ":" + to_string(Date.Getsecond());
	}
